...
def add_stt_noise(words):
    """Word-level STT noise after sentence is fully constructed."""
    noisy = []
    for w in words:
        r = random.random()
        if r < 0.05:
            ...
            noisy.append(w[:-1])   # drop last char
        elif r < 0.15:
            noisy.append(w)
            noisy.append(w)        # repeat
        else:
            noisy.append(w)
    return noisy
...
def generate_sample(i):
    ...
    for label in labels:
        ...
        # Insert placeholder token for alignment
        placeholder = f"<<ENT{i}_{label}>>"
        template = [
            "the", "user", "mentioned", placeholder,
            "in", "the", "conversation"
        ]
    ...
    # Add noise on token level BEFORE inserting entities
    noisy_tokens = add_stt_noise(template_tokens)

    # Rebuild noisy string, then substitute entities in final positions
    ...
    # Compute spans from exact substrings
    spans = []
    for ent in entities:
        ent_string = " ".join(ent["tokens"])
        label = ent["label"]
